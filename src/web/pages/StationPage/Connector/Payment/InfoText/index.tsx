    ...
    switch (powerType) {
      case POWER_TYPE.FULL:
        return paymentType === PAYMENT_TYPE.CARD
          ? `До старта зарядки с карты будет списана сумма, эквивалентная ${power} kWh. Сдача будет возвращена после завершения`
          : 'После завершения со счета будет списана сумма, соответствующая объему заправленной энергии';

      case POWER_TYPE.MIDDLE:
        return paymentType === PAYMENT_TYPE.CARD
          ? `До старта зарядки с карты будет списана сумма эквивалентная ${Math.round(
              power * 0.8,
